...
def main() -> int:
    ...
    if score_before is None:
        print('the accepted baseline carries no scoreboard figure, so the gate would be '
              'judging on local terms alone. Re-run with --init, or repair '
              f'{ACCEPTED_EVAL}. Refusing to score a patch against half a baseline.')
        return 1
