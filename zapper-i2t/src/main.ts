 ...
 *
 * Its only job is to put `createZapperModel()` in front of a deterministic
...
// A prop is framed on its own bounds, not on a floor-to-FRAME window: a 0.3 m pistol
...
try {
  ...
  try {
    scene.environment = createZapperEnvironment(renderer);
  } catch {
  ...
  try {
    ...
    const lights = createZapperLookDevLights();
  ...
  model.traverse((o) => {
    ...
    lows.push({ name: m.name || o.name || '(unnamed)', minY: +b.min.y.toFixed(4), maxY: +b.max.y.toFixed(4),
      w: +(b.max.x - b.min.x).toFixed(4), d: +(b.max.z - b.min.z).toFixed(4),
      ...
      x0: +b.min.x.toFixed(4), x1: +b.max.x.toFixed(4),
      z0: +b.min.z.toFixed(4), z1: +b.max.z.toFixed(4) });
  });
