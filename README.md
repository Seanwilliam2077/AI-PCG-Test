...
这一线把它照字面执行：**先冻结合同，再建模**。这个顺序是有意的——先声明再建造，才谈得上「事先声明的约束」。

模型已建完并按冻结的合同打过分：**60 个网格、21,560 三角形**。

![Zapper 六视图与四项实测指标](images/board-zapper.png)
...
| | 结果 | 条件 |
|---|---|---|
| 合同约束 | **32 / 32** | 32 条是 104 条可证伪行里能翻译成检查器 DSL 的部分，覆盖率在 `build_contract_json.py` 里显式报告，不是 104 条全过 |
| 装配树 | **VALID**，72 个命名部件，单根 | — |
| 关节几何有效 | **2 / 2** | 一把手枪就只有击锤和扳机两个铰接件。Nova3D 报的是 59 个关节，量级完全不同 |
| 编辑局部性 | **8 / 8** | 其中只有 2 个是**生成参数**（句柄是产出几何的那段代码里的一项）；另外 6 个是作用在命名子树上的变换，是**弱形式**，报告里按弱形式记 |

**首轮 32 条里有 5 条失败，5 条全部是检查表达式的缺陷，不是模型的。** `inside` 比较三个轴，
而同轴配对只该比两个（内衬比膛线长、套管比它外面的箍长，第三个轴必然"超出"）；`flush` 比的是
两个 `hi` 面，而贴合要比的是"A 的近端面对 B 的远端面"。补上 `concentric` 和 `meets` 两个算子之后
才是 32/32——**这些检查是看到失败之后才改的**，记在这里，不当成一次过。

### 八个编辑句柄

![八个编辑句柄中的四个，每个扫三档](images/board-handles.png)

每一格都是同一份 TypeScript 工厂，只改一个数字；整块板同一个 pixels-per-metre，所以图上的差别
就是模型上的差别。有意思的是**没变的部分**：菱形开口从 12 改到 20，套筒外壳的两个 rim 面移动
**0.00 mm**；枪口环从 2 改到 4，套筒轴向范围 179.8–212.0 mm 一微米不差。

真去实现 §8 还**证伪了合同自己的两行**，都是那次 25 条对抗审计没抓到的：

1. **H1 同时要求 `mid-band` 随枪管加长而平移、又把 `tube-fore` 的后端面钉死**——而那个后端面
   （110.3 mm）**就是 mid-band 自己的前端面**。两条在任何模型上都不可能同时成立，一动就在枪管上
   裂开一道 Δ 宽的缝。选了保住锚点、让 mid-band 不动，理由和取舍都写在 `handles.ts` 里。
2. **H4 的 move 列表只写了开口，漏了 struts**——而 struts 就是开口之间剩下的那些实心弧，
   改计数不可能不重建它们。

读一份约束和满足一份约束，会在不同的地方出错。这两条只有真去实现才撞得上。
